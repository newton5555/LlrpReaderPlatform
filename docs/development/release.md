...
- `LlrpSdk`：`2.0.3`
- `LlrpSdk.Extensions.Impinj`：`2.0.3`
- `LlrpSdk.Extensions.Zebra`：`2.0.3`
- `LlrpDevice.Virtual.Hosting`：`2.0.3`（`LlrpVirtualDevice.App.Wpf` 和 Blazor 虚拟设备挂件的直接依赖）
...
- Release 资产清单中的 `LlrpSdk/2.0.3` 和 `LlrpDevice.Virtual.Hosting/2.0.3` 类型为 `package`，不是 `project`；
- 发布版本号与平台版本声明一致；
