...
> 最新自动化基线：385 项全绿（Contracts 5、Services 194、Infrastructure 10、App.Wpf 134、Architecture 9、Extensions.Impinj 17、Extensions.Zebra 6、VirtualReader 10）。标准 Reader 未声明 GPIO 数量时，成功状态查询会将返回端口合并到运行时快照；这不替代 GPI 物理事件/触发验收。取消 `ActivateAsync`、Inventory Start 和短操作在 Probe/连接/扩展 Session 替换阶段会回收并重建干净 Session，下一次操作重新探测能力。
> 生成日期：以提交时为准。
> 本文档供接手的开发者在短时间内了解项目现状、关键设计、已知边界与下一步。

2026-08-19 文档状态收口：`LlrpReaderPlatform.App.Wpf` 作为真实 Reader 主客户端已完成首版交付，
`LlrpVirtualDevice.App.Wpf` 已完成报文级 TCP Virtual Reader 管理 UI 的首版实现，并通过
`LlrpDevice.Virtual.Hosting` NuGet 进入独立正式 Release ZIP。平台级 Virtual Reader Data Source 化仍是
...
dotnet build LlrpReaderPlatform.slnx   # 0 警告 0 错误
 dotnet test  LlrpReaderPlatform.slnx --no-build   # 385 项全绿
```

测试分布：Contracts.Tests 5、Services.Tests 194、Infrastructure.Tests 10、App.Wpf.Tests 134、Architecture.Tests 9、Extensions.Impinj.Tests 17、Extensions.Zebra.Tests 6、VirtualReader.Tests 10。
...
- 决策：`docs/decisions/`（ADR 0001~0018）
- 冻结仓库参考：`docs/legacy/README.md`

## 9. 交接注意事项

> 当前状态（2026-08-19）：P0～P7 首版代码和 P8 软件链路已完成；P8 的硬件现场剩余项、M2 R6 物理联动以及设备矩阵中的未覆盖能力继续按现场证据推进。报文级 Virtual Device Manager UI 已完成首版并进入独立正式 Release ZIP；平台级 Virtual Reader Data Source 仍是 ADR-0016 的长期未排期项。
...
- 本轮代码边界：Contracts 新增 Reader 端点归一化和 `PlatformOperationException`，程序化添加、SQLite、SDK 会话构造和 WPF IPv6 展示共享同一 Host 规则；同时保护 Start 返回与早到终止生命周期事件之间的状态竞态；Tag Logging 关闭时不会创建 Run 文件；全局寻卡部分失败状态按 Reader 名称和错误摘要定位。文中按日期保留的 304/307/311/312/318/336/356/357/360/378 等数字是历史阶段基线；当前测试分布为 Contracts 5、Services 194、Infrastructure 10、App.Wpf 134、Architecture 9、Extensions.Impinj 17、Extensions.Zebra 6、VirtualReader 10，完整基线为 385 项，构建 0 警告 0 错误。
...
验证结果：`dotnet build LlrpReaderPlatform.slnx --no-restore` 0 警告/0 错误；`dotnet test LlrpReaderPlatform.slnx --no-build` 385 项全绿；Markdown 本地链接和 `.slnx` XML 均通过检查。
