...
9. **P8：真机与兼容性**——软件链路和自动化回归已完成；R420、标准 Reader、多 Reader、断网/重启、GPI 物理事件及未覆盖写入项继续按设备矩阵进行现场验收。
10. **M1：多版本与多厂商推进**（dev 分支）——阶段 D、A、C0、B、C、E 已完成；阶段 F 的自动化收口已完成，剩余为对应硬件现场证据；范围、SDK 能力边界与缺口清单见[主计划 9.1](../llrp-framework-vision.md)。
11. **M2：寻卡联动上报设置**（ADR-0013）——阶段 R1～R5 已完成（build 0 错误，测试全绿），R6 物理联动验证待设备现场；当前状态以[主计划 9.2](../llrp-framework-vision.md)、[ADR-0013](../decisions/ADR-0013-report-capability-ownership.md)和[设备矩阵](../compatibility/device-matrix.md)为准。
## 长期规划（未排期）
...
Zebra 扩展/SDK 纳入架构依赖测试。当时自动化基线为 368 项全绿（Architecture 9、Impinj 17、
