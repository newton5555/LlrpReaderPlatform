...
- 状态：进行中（Planning）；**R1–R5 已完成**（build 0 错误，全量测试 349 项全绿：Contracts 5、Services 183、Impinj 17、Architecture 7、Infrastructure 10、App.Wpf 127）。**R6 真机部分完成**：R420（identity Mfr 25882/Model 2001002/FW 6.4.1.240，IP 已由 .134 变更为 .87）真实盘存相位字段 `impinj.rfPhaseAngle` 实测命中；Zebra FX9600（Mfr 161/Model 96008/FW 3.32.37.0，IP .88）完成 L1 身份连接；R6 剩余为 GPI 物理触发/多 Reader 并存/断网重启等现场项，见[设备矩阵](../compatibility/device-matrix.md)。
- 创建：2026-08
